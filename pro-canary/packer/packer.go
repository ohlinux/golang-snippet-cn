...
func main() {
	TarFile := "/tmp/test.tar.gz"
	src := "/Users/ajian/Baidu/Project/canary/codecenter"
	dstDir := "/tmp/"

	if err := TarGz(src, TarFile, false); err != nil {
		fmt.Println(err)
	}
...
func TarGz(src string, dstTar string, failIfExist bool) (err error) {
	// 清理路径字符串
	src = path.Clean(src)
